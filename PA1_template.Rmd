...
```
* _**Comment:**_
* _**The average of the total number of steps taken per day is `r round(steps.average)` and the median is `r round(steps.med)`.**_
...
```
* _**Comment:**_
* _**Interval no `r steps.average.top[,1]` contains the maximum number of steps (`r round(steps.average.top[,2])`) on average across all days.**_
...
```
* _**Comment:**_
* _**The total number of rows with NAs in the dataset is `r sumna`.**_
...
```
* _**Comments:**_
* _**Following the imputation, the average of the total number of steps taken per day is `r round(steps.average.new)` and the median is `r round(steps.med.new)`.**_
* _**So the new mean is greater than the old mean (`r round(steps.average)`).**_ 
* _**Also, the new median is greater than the old median (`r round(steps.med)`).**_
